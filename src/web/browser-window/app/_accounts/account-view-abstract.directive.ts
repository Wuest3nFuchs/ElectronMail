...
export abstract class AccountViewAbstractDirective extends AccountLoginAwareDirective implements OnDestroy {
    @Input({required: true})
    webViewSrc!: string;
    ...
    ngOnDestroy(): void {
        ...
        this.log("info", [nameof(AccountViewAbstractDirective.prototype.ngOnDestroy)]);
        this.action(
    ...
    private registerWebViewEventsHandlingOnce(webView: Electron.WebviewTag): void {
        // eslint-disable-next-line @typescript-eslint/unbound-method
        this.log("info", [nameof(AccountViewAbstractDirective.prototype.registerWebViewEventsHandlingOnce)]);
